...
void FSolveSecondPage::AddDiscScore(const OpenXLSX::XLWorksheet& fSheet, int iKeyPageNumber)
{
    ...
    for (auto row : fSheet.rows())
    {
        ...
        for (auto it : row.cells())
        {
            ...
            if (wsData != L"")
            {
                if (ptrGlobal->ptrConfig->arrKeyPage[iKeyPageNumber].arrHeader[0].count(wsData)) iIdAllow = x;
                if (ptrGlobal->ptrConfig->arrKeyPage[iKeyPageNumber].arrHeader[1].count(wsData)) iIdIndex = x;
                if (ptrGlobal->ptrConfig->arrKeyPage[iKeyPageNumber].arrHeader[2].count(wsData)) iIdLScore = x;
                if (ptrGlobal->ptrConfig->arrKeyPage[iKeyPageNumber].arrHeader[3].count(wsData)) iIdRScore = x;
...
double FSolveSecondPage::DFSCountingScore(FTreeElement* ptrThis)
{
    ...
    {
        if (ptrThis->bAllow) { return ptrThis->dSumScore; }
        else { return 0; }
    }

    double dScore = 0;

    for (auto it : ptrThis->arrChild) { dScore += DFSCountingScore(it); }
