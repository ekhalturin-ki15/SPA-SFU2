...
void FMetric::UpdateMetricBranch(FTreeMetric* ptrNowTree, set<vector<string>>& setIsTakenScore
	, const string& sName, const double& dScore)
{
...
void FMetric::UpdateCourseMetric(FTreeMetric* ptrRootTree, set<vector<string>>& setIsTakenScore,
	const vector<string>& arrRectUpdate, const double& dScore)
{
		...
		for (auto& [comp, arrInd] : it->mapComp)
		{
			for (const auto& ind : arrInd)
			{
				vector<smatch> matches{
					sregex_iterator{ALL(ind), fRegexHeaderIndicator},
						sregex_iterator{}
				};
				...
				for (auto sData : matches)
				{
					...
					UpdateCourseMetric(ptrTreeMetric->mapChild[sAllMetric], mapIsTakenScore
						, { sCompName , sCompNumber , sIndicatorNumber }, it->dSumScore);

					for (auto [iCourse, dScore] : it->mapCourseScore)
					{
						string sCourse = to_string(iCourse);
						UpdateCourseMetric(ptrTreeMetric->mapChild[sCourse], mapIsTakenScore
							, { sCompName , sCompNumber , sIndicatorNumber }, dScore);
					}
				}
