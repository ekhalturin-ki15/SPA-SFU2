...
//Использую структуру данных Бор, так как есть Компетенция, её индекс, её индикатор и так далее (сейчас вложеность 3, но нет смысла ограничивать)
struct FTreeMetric
{
	string sName = "-";
...
private:
	...
	void UpdateCourseMetric(FTreeMetric* ptrRootTree, set<vector<string>>& setIsTakenScore, 
	...
	void UpdateMetricBranch(FTreeMetric* ptrNowTree , set<vector<string>>& setIsTakenScore,
